...
from transfer.downstream.finetune.env.imports import pdb, torch, nn, CrossEntropyLoss, os, json
...
class PreTrainedModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        pdb.set_trace()
        if config is None:
            config, model_kwargs = cls.config_class.from_pretrained(pretrained_model_name_or_path, *model_args, cache_dir=cache_dir, return_unused_kwargs=True, force_download=force_download, **kwargs_config)
        else:
            model_kwargs = kwargs
            ...
            def load(module, prefix=''):
                ...
                if not prefix.startswith("head") or prefix.startswith("head.mlm"):
                    ...
                    if len(missing_keys) == 0:
                        print("WARNING {} missing keys in state_dict {}".format(prefix, missing_keys))
                module._load_from_state_dict(
                    state_dict, prefix, local_metadata, True, missing_keys, unexpected_keys, error_msgs)
                for name, child in module._modules.items():
                    #load_params_only_ls = kwargs.get("load_params_only_ls ")
                    not_load_params_ls = kwargs.get("not_load_params_ls") if kwargs.get("not_load_params_ls") is not None else []
        ...
        for key, _ in state_dict.items():
            ...
            for mapping_key, mapping_value in keys_mapping.items():
                if key.startswith(mapping_key):
                    ...
                    print("MODEL  replacing key {} from  loaded statedic  with {} ".format(mapping_key + key[len(mapping_key):],
