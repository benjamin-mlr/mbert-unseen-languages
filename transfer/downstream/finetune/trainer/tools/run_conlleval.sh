...
# changed how gold is defined !
if [ $name == "dev" ]; then
  echo "Starting $(dirname $0)/bilou2bio.py < ${system} > ${name}_system_bio.conll"
  $(dirname $0)/bilou2bio.py < ${system} > ${name}_system_bio.conll
  #$(dirname $0)/bilou2bio.py < $(dirname $0)/${gold} > ${name}_gold_bio.conll
  $(dirname $0)/bilou2bio.py < ${gold} > ${name}_gold_bio.conll
  ...
  paste ${gold} ${name}_system_bio.conll | cut -f1,2,3,4,8 > ${name}_conlleval_input.conll
else
